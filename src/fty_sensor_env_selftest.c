/*  =========================================================================
    fty_sensor_env_selftest.c - run selftests
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    { "libth", NULL, true, false, "libth_test" },
...
{
    test_item_t *item;
    printf ("Running fty-sensor-env selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (strcmp (item->testname, "private_classes") == 0)
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_SENSOR_ENV_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_sensor_env_private_selftest (verbose, item->subtest);
#endif // FTY_SENSOR_ENV_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  strcmp (argv [argn], "-h") == 0) {
            puts ("fty_sensor_env_selftest.c [options] ...");
    ...
    if (test) {
        ...
        if (!test->subtest)
            ...
            fty_sensor_env_private_selftest (verbose, test->subtest);
#endif // FTY_SENSOR_ENV_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
